...
'''
NUMBER_OF_REPRESENTER_POINTS = 25

'''
The number of locations at which we will draw function samples from the Gaussian process.
'''
NUMBER_OF_SAMPLE_LOCATIONS = NUMBER_OF_REPRESENTER_POINTS + 1
...
class EntropySearch(object):
    def __init__(self, comp, vals, gp, cost_gp=None):
        ...
        #these are the locations that we will use to draw function samples
        #the locations are the representer points AND the candidate
        #the FIRST entry will be the candidate
    ...
    def compute(self, candidate, compute_gradient = False):
        ...
        gain = 0
        self._func_sample_locations[0] = candidate
        log_proposal_vals = np.zeros(NUMBER_OF_SAMPLE_LOCATIONS)
        pmin = self._compute_pmin_bins()
        ...
        for i in range(0, NUMBER_OF_SAMPLE_LOCATIONS):
            log_proposal_vals[i] = self._log_proposal_measure(self._func_sample_locations[i])
        log_proposal = np.dot(log_proposal_vals, pmin)
        kl_divergence = entropy_pmin - log_proposal
        gain = gain + kl_divergence
        return gain

    def _compute_pmin_bins(self):
        '''
        ...
        '''
        pmin = np.zeros(NUMBER_OF_SAMPLE_LOCATIONS)
        for omega in self._Omega:
            vals = self._gp.drawJointSample(self._func_sample_locations, omega)
            ...
            #indepentend of how many mins we have only the first wins!
            pmin[mins[0][0]] += 1#/(number_of_mins)
